...
investmentsRouter.post("/refresh-daily", async (req, res, next) => {
  try {
    ...
    const result = await refreshDailyQuotesForTwelveData();
...
investmentsRouter.post("/instruments", async (req, res, next) => {
  try {
    const user = await assertAdmin(req, res);
    if (!user) return;
...
investmentsRouter.post("/instruments/bulk", async (req, res, next) => {
  try {
