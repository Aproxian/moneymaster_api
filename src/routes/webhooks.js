...
webhooksRouter.post("/revenuecat", async (req, res, next) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      // Idempotency: only store the delivery marker in the same transaction as its effects.
      const already = await tx.revenueCatEvent.findUnique({ where: { id: eventId } });
      if (already) {
      ...
      if (anonymous || !concernsFullAccess(event)) {
        ...
        return { ok: true, applied: false, reason: "no_matching_user_or_entitlement" };
      }
      ...
      if (state.active === null) {
        ...
        await tx.revenueCatEvent.create({ data: { id: eventId, userId: appUserId, type } });
        return { ok: true, applied: false, reason: "indeterminate", type };
      }
      ...
      return { ok: true, applied: updated.count > 0, type };
    });
