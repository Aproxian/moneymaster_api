...
const { applyPremiumStateEvent } = require("../src/routes/webhooks");
...
test("premium webhook idempotency marker rolls back when the user update fails", async () => {
...
test("annual expiration does not overwrite an existing lifetime purchase", async () => {
  const prisma = makeMockPrisma();
  ...
  const user = prisma.state.users.get("user_1");
  assert.equal(result.applied, false);
  assert.equal(result.reason, "stale_downgrade_ignored");
  assert.equal(prisma.state.events.has(event.id), true);
  assert.equal(user.premiumActive, true);
  assert.equal(user.premiumIsLifetime, true);
  assert.equal(user.premiumProductId, PRODUCT_ID_LIFETIME);
});

test("customer-support lifetime cancellation revokes access and clears lifetime bypass", async () => {
  const prisma = makeMockPrisma();
  prisma.state.users.set("user_1", {
    id: "user_1",
    premiumActive: true,
  ...
  const user = prisma.state.users.get("user_1");
  assert.equal(result.applied, true);
  assert.equal(user.premiumActive, false);
  assert.equal(user.premiumIsLifetime, false);
  assert.equal(hasActivePremium(user), false);
});
